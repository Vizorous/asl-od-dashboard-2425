            ...
            if idx < num_kpis:
                cols[j].markdown(
                    ...
                    <div style="
                        ...
                        text-align: center;
                    ">
                        <p style="margin: 0;">{title}</p> <!-- Add margin: 0; to remove default margin -->
                        <h3 style="margin: 0;">{int(kpi_values[idx])}</h3> <!-- Add margin: 0; to remove default margin -->
                    </div>
                    """
                , unsafe_allow_html=True)
...
filtered_data_core=data_core[ (data_core['month_name'] == selected_month)]
selected_function = st.selectbox('Select Function',filtered_data_core['Function'].unique() )
